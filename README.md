...
- [x] 1) Add intermediary WD points to explore how fairness metrics degrade more granularly (seeing if it sharply drops off at a certain point (threshold) or if it deteriorates linearly)
  - x ] a) Recreate more train sets withholding and adding different properties (geography, temporal/years, income, etc) to create more WD data points
  - [x] b) Retrain and re-test (KEEP DATASETS IN PICKLE FOR FUTURE USE WITH DIFF CLASSIFIER/FAIRNESS METHODS)
  - [x] c) Graph and analyze the behavior of the fairness metrics and see if any conclusions can be made. Compare the degradation of fairness metrics (e.g., max calibration gap, equalized odds difference) across different sensitive attributes (ie race, geography, income groups). This might pinpoint which attributes are driving fairness metric deterioration under specific distribution shifts.

- [x] 2) Use different classifiers (neural networks) that can hopefully take advantage of the history of the features or do some extra feature creating to display the difference in income, health, etc over the past couple years for each individual. See if that can bump up accuracy
  - [x] a) Use previous datasets of intermediary WDs to see how fairness changes with other classifiers

- [ ] 3) Investigate group base rates and feature interactions to understand why one metric may behave differently than the other fairness metrics. Look at relationships between the metrics to understand their interdependencies and underlying drivers.
  ...
  - [ ] c) Identify what features impact metrics most
  - [ ] d) Look at how subgroups (ie by race and income combined) are affected by distribution shifts. Try methods like bias scans or intersectional subgroup fairness metrics to identify where and why disparities occur​.


- [x] 4) Use fairness-enhancing techniques and assess their effectiveness under distribution shiftss
  - [x] a) Use previous datasets of intermediary WDs to see how fairness changes

- [ ] 5) Modeling Fairness-Accuracy Trade-offs
  - [ ] a) Evaluate whether fairness improvements come at the cost of accuracy under distribution shifts.
